...
This guide provides practical examples of how to use the core `Repository` object in `codekite` to interact with your codebase.

## Initializing a `Repository`

First, create an instance of the `Repository` class, pointing it to your code. `codekite` can work with local directories or clone remote Git repositories.
This is the starting point for any analysis, giving `codekite` access to the codebase.
...
`codekite` can clone a public or private Git repository. For private repos, provide a GitHub token.
...
When cloning remote repositories, `codekite` caches them locally to speed up subsequent initializations. By default, caches are stored in a temporary directory. You can specify a persistent cache directory:
...
`codekite` provides utilities to prepare code snippets for large language models.
...
`codekite` includes an alpha feature for generating natural language summaries (like dynamic docstrings) for code elements (files, functions, classes) using a configured Large Language Model (LLM). This can be useful for:
...
**Note:** This feature is currently in **alpha**. The API may change, and it requires an LLM (e.g., via OpenAI, Anthropic) to be configured for `codekite` to use for summarization.
...
```python
from codekite import Repository
from codekite.docstring_indexer import DocstringIndexer
from codekite.models import OpenAI # Or your preferred LLM provider model
...
`codekite` can export the gathered information (file tree, symbols, index, usages) to JSON files for use in other tools or offline analysis.
Persist the results of your analysis or integrate `codekite`'s findings into other development workflows.
