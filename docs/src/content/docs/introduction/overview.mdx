...
`kit` is **free and open source** with a permissive MIT license. Check it out on [GitHub](https://github.com/shaneholloman/codekit).
...
```sh
# Basic installation, it's currently `codekit` on pypi
pip install codekit

# With OpenAI support
pip install codekit[openai]

# With all features (OpenAI, Anthropic, Google, vector search, API)
pip install codekit[all]
```
