...
## The Workflow: Analyze with `codekite`, Act with `Supersonic`

A typical integration pattern looks like this:

1.  **Analyze Code with `codekite`**: Use `codekite.Repository` methods like `extract_symbols`, `find_symbol_usages`, or `search_semantic` to understand the codebase or identify areas for modification.
...
Imagine an AI tool that uses `codekite` to analyze a Python file, identifies a potential refactoring, generates the improved code, and then uses `Supersonic` to create a PR.
...
# Assume codekite.Repository is initialized with a local path
LOCAL_REPO_PATH = "/path/to/your/local/repo/clone"
# repo_analyzer = codekite.Repository(LOCAL_REPO_PATH)
# Note: codekite analysis methods like extract_symbols would still be used here in a real scenario.

# Assume 'ai_generate_refactoring' is your function that uses an LLM
# potentially fed with context from codekite (not shown here for brevity)
...
This example illustrates how `codekite`'s analytical strengths can be combined with `Supersonic`'s action-oriented PR capabilities to build powerful code automation.
