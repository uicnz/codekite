...
This tutorial demonstrates how to visualize the import dependency graph of a Python codebase using `codekite`. The output is in DOT format, which you can render with Graphviz to generate visual diagrams.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dependency graph visualizer using codekite.")
...
Visualizing dependencies helps you understand, refactor, and document complex codebases. With `codekite` and Graphviz, you can generate these graphs automatically as your project evolves.
