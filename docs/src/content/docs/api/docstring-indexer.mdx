...
*(defined in `codekite/docstring_indexer.py`)*

```python
from codekite import Repository, Summarizer
from codekite.docstring_indexer import DocstringIndexer, EmbedFn # EmbedFn is Optional[Callable[[str], List[float]]]
from codekite.vector_searcher import VectorDBBackend # Optional
...
#     return [0.1, 0.2, ...]
#
# from codekite.vector_searcher import ChromaDBBackend
...
*   **`repo`** (`Repository`, required):
    An instance of `codekite.Repository` pointing to the codebase to be indexed.
*   **`summarizer`** (`Summarizer`, required):
    An instance of `codekite.Summarizer` used to generate summaries for code symbols or files.
