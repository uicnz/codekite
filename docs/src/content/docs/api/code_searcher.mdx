...
```python
from codekite.code_searcher import CodeSearcher

searcher = CodeSearcher(repo_path="/path/to/your/repo")
# Or, if you have a codekite.Repository object:
# searcher = repo.get_code_searcher()
```

<Aside type="note">
  If you are using the `codekite.Repository` object, you can obtain a `CodeSearcher` instance via `repo.get_code_searcher()` which comes pre-configured with the repository path.
</Aside>

## `SearchOptions` Dataclass

The `search_text` method uses a `SearchOptions` dataclass to control search behavior. You can import it from `codekite.code_searcher`.

```python
from codekite.code_searcher import SearchOptions
```
...
```python
from codekite.code_searcher import CodeSearcher, SearchOptions
