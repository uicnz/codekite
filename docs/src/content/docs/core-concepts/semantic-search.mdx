...
codekite supports semantic code search using vector embeddings and ChromaDB.
...
# Example output:
# src/codekite/auth.py login function def login(...): ...
# src/codekite/config.py AUTH_CONFIG variable AUTH_CONFIG = {...}

### Choosing an Embedding Model

`codekite` is model-agnostic: pass any function `str -> List[float]`.
...
The index lives under `.codekite/vector_db` by default (one Chroma collection per path).

```python
vs = repo.get_vector_searcher(embed_fn, persist_dir=".codekite/my_index")
vs.build_index()
# … later …
searcher = repo.get_vector_searcher(embed_fn, persist_dir=".codekite/my_index")
...
```python
from codekite import ContextAssembler
...
* Indexing a very large monorepo may take minutes: consider running on CI and committing `.codekite/vector_db`.
...
<Aside type="note">
  The CLI (`codekite search` / `codekite serve`) currently performs **text** search only. A semantic variant is planned.
</Aside>
