---
title: Docstring Vector Indexing
---
...
* **Provider-agnostic** - Works with any LLM supported by `codekite.Summarizer` (OpenAI, Anthropic, Google…).
...
```python
from codekite import Repository, DocstringIndexer, Summarizer
from codekite.llms.openai import OpenAIConfig # For configuring the summarization LLM
...
# from codekite import SummarySearcher
...
```python
from codekite import Repository, DocstringIndexer, Summarizer
from codekite.llms.openai import OpenAIConfig
...
_(Note for codekite Developers: Consider adding a backend-agnostic `indexer.inspect_entries()` method for easier inspection in the future)._
...
# 3. Build the index (stored in .codekite/docstring_db)
indexer = codekite.DocstringIndexer(repo, summarizer)
indexer.build()
...
`codekite.vector_searcher.VectorDBBackend`.  The default backend is
[`Chroma`](https://docs.trychroma.com/) and lives in
`.codekite/docstring_db/` inside your repo.
