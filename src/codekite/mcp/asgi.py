#!/usr/bin/env python
"""
ASGI-compatible MCP server for CodeKite.

This implementation follows best practices for FastMCP servers,
allowing integration with Starlette, FastAPI, or other ASGI applications.
"""
...
mcp = FastMCP(
    name="CodeKite MCP",
    description="CodeKite Model Context Protocol Server",
...
class Repository:
    ...
    def _clone_repository(self, url: str):
    ...
    def _get_file_tree(self) -> List[Dict[str, str]]:
...
async def codekite_get_repo_structure(repo_id: str) -> Dict[str, Any]:
    """
    Get repository structure.
...
async def codekite_get_repo_summary(repo_id: str) -> Dict[str, Any]:
    """
...
async def codekite_get_repo_docstrings(repo_id: str) -> List[Dict[str, str]]:
    """
...
async def health_check(request: Request) -> JSONResponse:
    ...
    return JSONResponse({
        "status": "healthy",
        "service": "CodeKite MCP Server",
        "version": "2.0.0",
        "repositories": len(_repos)
    })

# Create Starlette applications for different transport protocols
...
def create_sse_app(path: str = "/sse", message_path: str = "/messages/") -> Starlette:
    """Create a Starlette app for SSE transport."""
    return mcp.sse_app(path=path, message_path=message_path)

# Main ASGI app that can be mounted in other applications
def create_app() -> Starlette:
    """
    Create the main ASGI application.

    This app can be mounted in other Starlette or FastAPI applications
    or run standalone with uvicorn.
    """
    # Create the ASGI app using streamable_http_app method
    mcp_app = create_streamable_http_app(path="/mcp")

    # For standalone use, we create a simple Starlette app that contains our MCP app
    app = Starlette(
        routes=[
            Mount("/", app=mcp_app),
        ],
        # Important: We must pass the lifespan context to properly initialize the session manager
...
# For running standalone with uvicorn
async def run_server(
    host: str = "0.0.0.0",
    port: int = 8000,
    log_level: str = "info"
):
    """Run the server in standalone mode using uvicorn."""
    import uvicorn
